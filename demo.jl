...
history, t, system = simulate(
    ...
    options     = SimOptions(;
        log            = Logs.HDF5LogOptions(log_file),    # Logs to a portable HDF5 file.
        monitors       = [Monitors.ProgressBarOptions(),], # Show a progress bar.
        time_dimension = "Time" => "s",                    # Puts time units on plot labels.
    ),
)
